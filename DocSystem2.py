...
def agregar_credenciales(usuario, contraseña, archivo_json):
    ...
    if not os.path.exists("Users"):
        os.mkdir("Users")
    user_folder = os.path.join("Users", usuario)
    if not os.path.exists(user_folder):
        os.mkdir(user_folder)

    # Crear subcarpetas separadas para claves y documentos
    carpeta_documentos = os.path.join(user_folder, "documentos")
    carpeta_claves = os.path.join(user_folder, "claves")
    os.makedirs(carpeta_documentos, exist_ok=True)
    os.makedirs(carpeta_claves, exist_ok=True)

    # Generar y guardar las claves
    ruta_clave_publica, ruta_clave_privada = generar_par_claves(usuario, contraseña)

    # Agregar rutas de claves a las credenciales
    nueva_credencial.update({
        "ruta_clave_publica": ruta_clave_publica,
    ...
    messagebox.showinfo("Éxito", f"Usuario {usuario} registrado correctamente con par de claves.")
...
def ver_carpeta(usuario):
    ...
    ruta_carpeta = os.path.join("Users", usuario, "documentos")
